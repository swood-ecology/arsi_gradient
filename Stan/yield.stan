...
model {
  ...
  // Run model
  y ~ lognormal(fert_std*beta_std[1] + maom_std*beta_std[2] + pom_std*beta_std[3] + pH_std*beta_std[4] + trd_std*beta_std[5] + alpha_std,sigma_std);
}
generated quantities {
  ...
  real beta_trd = beta_std[5] * sd(y) / sd(trd);
  real<lower=0> sigma = sd(y) * sigma_std;

  // Predict data
  vector[N] y_tilde;
  for (n in 1:N)
    y_tilde[n] = lognormal_rng(fert_std[n]*beta_std[1] + maom_std[n]*beta_std[2] + pom_std[n]*beta_std[3] + pH_std[n]*beta_std[4] + trd_std[n]*beta_std[5] + alpha_std,sigma_std);
}
